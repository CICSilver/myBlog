...
class DiaryRouteTest(unittest.TestCase):
    ...
    def test_activity_has_counts_and_detail_links_but_no_private_content(self):
        ...
        self.assertEqual(len(payload["overview"]), 12)
        self.assertEqual(payload["overview"][today.month - 1]["recorded_days"], 1)
        self.assertEqual(payload["char_total"], 400)
        self.assertIn("data-diary-calendar", page)
        self.assertIn('data-calendar-collapse aria-expanded="true"', page)
        self.assertIn(f'href="/diary/{today.year}/{today.month}/{today.day}" title="', page)
        self.assertIn("is-current-week", page)
        self.assertIn("diary-calendar.js", page)
        self.assertIn("400</b><span>字 · 今年累计", page)
        self.assertIn("追上《狂人日记》", page)
